...
<%
    // 게시판 리스트 처리 - 페이징
    // 1. 전체게시물 수 처리 (bdcnt : 1072)
    // 2. 페이지당 보여줄 게시물 수 지정 (=perPage : 10)
    // 3. 총 페이지 수 계산 (=> 107 + 1 )
    // 4. 현재 페이지 번호 (cp, )
    // ex) list.do?cp=1 : 1072 ~ 1063
    // ex) list.do?cp=1 : 1062 ~ 1053
    // ex) list.do?cp=1 : 1052 ~ 1043
    // ...
...
<%
    ...
    // 페이지별로 계산해서 출력해야함
    // 전체 게시물 수 : 1072
    // cp=1 : startbno = 1072, endbno = 1063
    // cp=2 : startbno = 1062, endbno = 1053
    // cp=3 : startbno = 1052, endbno = 1043
    // cp=? : startbno = y
